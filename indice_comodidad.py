...
lista_ordenada=sorted(os.scandir("/home/arw/nc/wrfprd/d03"), key=lambda f: f.name.lower())
dim = len(lista_ordenada)
extension = range(0,dim)

j = 1
for i in extension:
    archivo = lista_ordenada[i].name
    #Se abre el netcdf
    df = xr.open_dataset("/home/arw/nc/wrfprd/d03/"+archivo)
    nc = Dataset("/home/arw/nc/wrfprd/d03/"+archivo)
    ...
    #shp = gpd.read_file('/home/arw/shape/mundo.shp')
    #-------------------------------------------------------------------------------------------------------------
    #-------------------------------------------------------------------------------------------------------------
    #Extraigo las variables de forma directa
    #temp = df['T2'][0,:,:]-273.15
    #hum = df['RH02'][0,:,:]#Extraigo la fecha y tiempo como un string
    time_stamp = df['XTIME'][:]
    ...
    #-------------------------------------------------------------------------------------------------------------
    #-------------------------------------------------------------------------------------------------------------
    #Genero un plot#Defino la palate de colores
    ...
    fig, ax = plt.subplots(figsize=(12,10))#,facecolor="#323943")# Establecer el color de los textos de los ejes a blanco
    #ax.xaxis.label.set_color('white')
    #ax.yaxis.label.set_color('white')# Establecer el color de los números de latitud y longitud a blanco
    #ax.xaxis.set_tick_params(labelcolor='white')
    #ax.yaxis.set_tick_params(labelcolor='white')#Creo el contorno usando la malla de puntos de latitudes y longitudes junto con la variable a dibujar
    ...
    #cbar.solids.set_edgecolor("face")#Le añado una etiqueta
    #cbar.set_label("Mapa de prueba", labelpad = +1, fontsize=18)
    #Se añade un titulo
    texto = "Índice de comodidad de Terjung adaptado a la sensación térmica para: \n" + tiempo + "\n Modelo WRF miembro C"
    ...
    plt.axis("off")
    #plt.imshow(logo)
    nombre_mapa = "indice_confort_"+str(i)+".png"
    plt.savefig(nombre_mapa)
    #Se muestra el plot
    #plt.show()
    #-------------------------------------------------------------------------------------------------------------